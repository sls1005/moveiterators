...
iterator itemsBorrowedFrom*[T](s: var seq[T]): T {.inline.} =
  ...
  ## Any attempt to modify `s` (including trying to `add` or assign to it) within the loop, except by using an address that's taken from an element before the first iteration, will not have an effect that lasts after the end of the last iteration.
  ##
...
iterator mutableItemsBorrowedFrom*[T](s: var seq[T]): var T {.inline.} =
  ...
  ## anyone trying to access `s` will instead get an empty `seq`, and the attempt to access any of its elements could instead cause an `IndexDefect`;
  ## this lasts until the last iteration is finished, and after that, `s` will be restored.
  ## Any attempt to modify `s` (including trying to `add` or assign to it) within the loop (except by using the yielded variable or an address that's taken from an element before the first iteration or from a yielded variable) will not have an effect that lasts after the end of the last iteration.
